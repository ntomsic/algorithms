...
def pacific_atlantic(matrix):
    ...
    if not _m:
        return []
    res = []
    atlantic = [[False for _ in range(_n)] for _ in range(_m)]
    pacific = [[False for _ in range(_n)] for _ in range(_m)]
    for i in range(_n):
        dfs(pacific, matrix, float("-inf"), i, 0)
        dfs(atlantic, matrix, float("-inf"), i, _m - 1)
    for i in range(_m):
        dfs(pacific, matrix, float("-inf"), 0, i)
        dfs(atlantic, matrix, float("-inf"), _n - 1, i)
    for i in range(_n):
        for j in range(_m):
