...
class Edge:
    def __init__(self, _u, _v, weight):
        self.u = _u
        self.v = _v
        self.weight = weight
...
class DisjointSet:
    ...
    def merge_set(self, _a, _b):
        ...
        if self.size[_a] < self.size[_b]:
            self.parent[_a] = _b  # Merge set(a) and set(b)
            self.size[_b] += self.size[_a]  # Add size of old set(a) to set(b)
        else:
            self.parent[_b] = _a  # Merge set(b) and set(a)
            self.size[_a] += self.size[_b]  # Add size of old set(b) to set(a)

    def find_set(self, _a):
        if self.parent[_a] != _a:
            ...
            self.parent[_a] = self.find_set(self.parent[_a])

        # node <a> it's the set root, so we can return that index
        return self.parent[_a]


def kruskal(_n, edges, d_s):
    # Args:
    #   n (int): Number of vertices in the graph
    ...
    for edge in edges:
        set_u = d_s.find_set(edge.U)  # Set of the node <u>
        set_v = d_s.find_set(edge.V)  # Set of the node <v>
        if set_u != set_v:
            d_s.merge_set(set_u, set_v)
            mst.append(edge)
            if len(mst) == _n - 1:
